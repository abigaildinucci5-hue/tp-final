...
export const fetchHabitacionesPopulares = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await habitacionesService.obtenerHabitacionesPopulares();
      const habitaciones = response.data?.data || response.data || [];
      return habitaciones.slice(0, 8);
    } catch (error) {
      return rejectWithValue(error.response?.data || 'Error al obtener habitaciones');
    }
  }
);
...
const habitacionesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // ✅ Fetch Habitaciones Populares — CORREGIDO
    builder
      ...
      .addCase(fetchHabitacionesPopulares.fulfilled, (state, action) => {
        state.loading = false;
        const habitaciones = Array.isArray(action.payload)
          ? action.payload
          : action.payload?.data || [];
        state.habitacionesPopulares = habitaciones.map((h) => ({
          ...h,
          id: h.id_habitacion,
        }));
      })
      .addCase(fetchHabitacionesPopulares.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.mensaje || 'Error al cargar habitaciones';
      });
  },
});
